...
CREATE OR REPLACE VIEW vactivosf AS
    SELECT a.icode AS codigo, a.iname AS nombre, a.artpeso AS peso, a.artcosto AS costo, a.isiva AS carga_iva, 
    		a.itag AS codigo_alt, u.descripda AS unidad, c.descripda AS marca, g.descripda AS grupo, 
    		s.descripda AS subgrupo, m.moname, a.iditem 
...
ALTER TABLE public.vactivosf OWNER TO postgres;
...
CREATE VIEW vsecudoc AS 
 SELECT DISTINCT e.idsecudoc, e.iddocu, e.idautosri, e.nota, d.cretri, d.ivaxitem, d.lispre, a.numauto, a.fcaducidad, 
 	...
 	d.swdesc, d.swcosto, d.swkar, d.swconta, d.swprint, d.swclose, d.swcostov, d.tipcosto, d.idsritabla, i.descripda AS tipo, 
 	s.descripda AS subtipo, d.clasedoc, e.estado, u.numese AS numactual, u.idsecu, d.ajucon, 
...
CREATE  VIEW vdocuments AS 
 ...
 d.flete, d.subconiva, tipitem, d.chofer, f.sname as chname, f.scedula as chcedula, f.sruc as chruc, s.stag, 
 ...
 d.isreversado, d.concepto, k.afecha AS feccon, b.descripda AS bodega, z.tipsaldo, z.iddocu, z.nomdoc, z.dtag, 
 z.swaneiva, z.tipnum, z.swkar, z.swconta, z.idsritabla, z.swcostov, z.ajucon, d.serie, d.numauto, d.fcaducidad, 
 s.scel, s.stelf,  s.saddr, s.saddr2, s.scode, s.ssri, s.sname, s.sruc, s.scedula, ci.descripda AS sciudad, 
 s.spasaporte, j.sname AS vendedor, d.caja, d.efectivo, 
 u.sname AS nombreu, monedas.moname AS moneda, d.pflag, d.dflag, d.idws, q.reporte, q.proceso, q.impresora, z.cretri, 
 a.sname as cajero, 
 t.scode as codter, t.sname as nombret, z.isdevo, z.istransfer, z.documentos,  
 r.sname as recaudador, x.descripda AS tipdocextraj, s.issocio
   FROM documents d
...
CREATE  VIEW varti AS 
 SELECT a.icode AS codigo, a.iname AS nombre, a.artpeso AS peso, a.artcosto AS costo, a.saldocon, a.isiva AS carga_iva, 
 	...
 	s.descripda AS subgrupo, m.moname, a.iditem, a.artgrupo, iunidad2, v.descripda as unidad2, a.isformato, 
 	p.descripda as promocion, b.descripda as ubicacion, b.iddato as codubi, a.artsgrupo, a.costopro 
   FROM items a
   LEFT JOIN detagru u ON a.iunidad = u.iddato
   LEFT JOIN monedas m ON a.imoney = m.mocode
   LEFT JOIN detagru c ON a.artmarca = c.iddato
...
CREATE VIEW varticulos AS
    SELECT a.iname, a.icode AS auxcon, f.descripda AS genero, a.iname AS auxiliar, a.artpeso, a.artsell, a.artcosto, a.costopro, 
    ...
    g.descripda AS grupo, s.descripda AS subgrupo, m.moname, a.iditem, a.artgrupo, a.iunidad, a.iunidad2, a.iunidad3, a.artsgrupo
    FROM ((((((((items a LEFT JOIN detagru u ON ((a.iunidad = u.iddato))) 
...
CREATE  VIEW vitems AS 
 SELECT a.iditem, a.artgrupo, a.artsgrupo, a.artipo, a.artmarca, a.artpeso, a.artsell, a.artcosto, a.artfob, a.artcosto2, 
 	...
 	a.costopro, a.isserie, a.isembala, a.ubica, b.descripda as ubides, a.codbar, s.descripda as subgrupo, a.inactivo
   FROM items a
   LEFT JOIN detagru u ON a.iunidad = u.iddato
   LEFT JOIN monedas m ON a.imoney = m.mocode
   LEFT JOIN detagru c ON a.artmarca = c.iddato
...
CREATE VIEW vdetadoci AS
    SELECT t.code, i.iditem, i.icode, i.iname, i.marca, i.grupo, i.genero, i.itipo, i.iname AS auxiliar, i.icode AS auxcon, 
    		t.punitario, i.unidad, i.artgrupo, t.qty, t.valor AS subtot, t.valconiva, t.descuento, t.qtyactivo, t.isiva, 
    		t.artcosto, i.depre, t.pordes, t.impuesto, t.secuencia, t.idcontenedor, i.isiva as isivai, i.codbar, i.artpeso
...
CREATE VIEW vdocmul AS
    SELECT d.numero, d.nrodoc, o.nomdoc, d.fecha, d.iddocu,  d.subtotal, d.idsujeto, d.codusu, d.observac, d.isprint, d.isanulado, 
    d.fecgra, d.isgenerado, d.iscancelado, d.isaccount, o.dtag, s.scode AS scodeb, 
    CASE WHEN (char_length(btrim((s.sname)::text, ''::text)) = 0) THEN s.ssri ELSE s.sname END AS beneficiario, 
    t.idsujeto AS idsujetod, CASE WHEN (char_length(btrim((u.sname)::text, ''::text)) = 0) THEN u.ssri ELSE u.sname END AS sname, 
    u.scode, t.valor, t.cant, (t.valor * t.cant) AS subtotald, t.code, d.pdocode
    FROM (((((docmul d LEFT JOIN sujetos s ON ((d.idsujeto = s.idsujeto))) LEFT JOIN docuse o ON ((d.iddocu = o.iddocu))) LEFT JOIN periodos p ON ((d.pdocode = p.pdocode))) LEFT JOIN docmuld t ON ((d.nrodoc = t.nrodoc))) LEFT JOIN sujetos u ON ((t.idsujeto = u.idsujeto)));
...
CREATE VIEW vdocub AS
    ...
    t.isprint AS isprintb, t.tipodoc, t.fecpag, t.isposfec, t.isconpos, t.isdet, t.secuencia, r.dtag, r.nomdoc, t.fecha, t.isaccount   
...
CREATE  VIEW vdocui AS 
 SELECT t.code, i.iditem, i.icode, i.iname, i.marca, i.grupo, i.itag, i.genero, i.itipo, i.iname AS auxiliar, i.ubides, 
 	...
 	i.isformato, t.secuencia, t.idcontenedor, t.fecven, i.isperecible, t.qtypromo, i.artpeso, i.isserie, t.serie1, 
 	t.serie2, t.modelo, t.anio, t.cilindraje, t.observ, c.descripda as color, i.isiva as isivai, i.codbar, i.ubica
   FROM detadoci t
   LEFT JOIN vitems i ON t.iditem = i.iditem 
   left join detagru c on (t.color=c.iddato);
...
CREATE  VIEW vdocumentg AS 
 SELECT DISTINCT d.code, s.sname AS nombre, d.montototal, v.nomdoc AS documento, d.num, s.scode AS cod_persona, s.ssri AS sri, 
 	s.sruc AS ruc, s.scedula AS cedula, r.sname AS vendedor, d.fecha, d.linkdoc, d.valcuota, d.numcuota, d.poriva, 
 	d.subtotal, d.ivavalor, d.ivabienes,  d.saldo, d.tmpsaldo AS saldo_inic, d.isprint AS impreso, 
 	d.isaccount AS contabilizado, d.ispagado AS cancelado, d.isanulado AS anulado, d.islock, d.isautorizado AS autorizado, 
 	d.concepto, k.afecha AS fec_conta, d.ffrom AS desde, d.fto AS hasta,  b.descripda AS bodega, d.caja, d.efectivo, 
 	v.dtag, d.pdocode, z.isdevo, z.istransfer, d.chofer, d.fecgra AS fec_grabac 
   FROM documents d
   LEFT JOIN detagru b ON d.userstore = b.iddato
   LEFT JOIN asientosdia k ON d.code = k.code
   LEFT JOIN vsecudoc z ON d.idsecudoc = z.idsecudoc
...
CREATE  VIEW vdocusmall AS 
 SELECT DISTINCT d.code, d.userstore, d.usercaja, d.num, d.cliente, d.fecha, d.fto, d.pdocode, d.idsecudoc, d.seller, 
 		...
 		d.flete, d.subtotal, d.subconiva, d.chofer, u.stag, d.caja, d.efectivo, 
 		...
 		u.scedula, u.saddr, d.pflag, d.dflag, d.idws, v.cretri, v.swconta, v.swcostov, d.duracion, d.isreversado, d.idugra, 
 		t.sname as nombret, v.isdevo, v.istransfer, d.casher, d.sujter, v.ajucon, u.codprin, d.fecgra
   FROM documents d
...
CREATE OR REPLACE VIEW vdocuret AS 
 SELECT d.code, d.fecharet AS fecha, d.numsecue1 AS numero, d.montoret, d.numserie1 AS seriedoc, 
	...
	s.descrip AS tipdoc, u.fecha AS feccom, u.sname, u.ssri, u.sruc, u.scedula, u.saddr, u.sciudad, 
	u.stelf, r.rubname, r.rubtab, r.valcal, c.valor, c.basecal AS monto, v.nomdoc, i.codigo as codigoret, d.fimpresion
   FROM aneiva d
   LEFT JOIN vdocuments u ON d.code = u.code
   LEFT JOIN cobros c ON c.code = d.code
  ...
  WHERE d.secueret>0 and r.isdocret ;
...
CREATE VIEW vdocuse AS 
 ...
 i.descripda AS tipo, s.descripda AS subtipo, r.nomdoc AS reversodoc, p.cuenta, d.ajucon, 
...
CREATE OR REPLACE VIEW vempleados AS
 SELECT s.scode, 
        CASE
            WHEN char_length(btrim(s.sname::text, ''::text)) = 0 THEN s.ssri
            ELSE s.sname
        END AS sname, s.scedula, s.sruc, e.emsexo, e.emcargo, c.descripda AS cargo, e.emsueldo, 
		e.emnumcar, v.descripda AS estciv, e.emnumiess, e.emfecing, e.emfecnac, 
		e.emfecsal, e.emcodocuiess, e.emnumcta, e.emtipcta, e.emcoddep, d.descripda AS departamento, 
		e.emisiess, t.descripda AS titulo, e.emisactivo, e.emhoex, s.idsujeto, 
		e.emarea, g.descripda AS areatrab
   FROM empleados e
   LEFT JOIN sujetos s ON e.idsujeto = s.idsujeto
   ...
   LEFT JOIN detagru g ON e.emarea = g.iddato
  WHERE s.isempleado;
...
CREATE VIEW vperiodos AS
    SELECT p.pdocode, p.fecini, p.fecfin, p.moneda, p.nivelplan, p.isupdate, p.isopen, 
...
CREATE  VIEW vpreciosv AS 
 SELECT c.descripda AS clientes, i.icode, i.iname, i.artcosto, i.costopro, i.artgrupo, 
 	i.artmarca, i.itag, p.preciov, p.idlista, p.iditem, p.pordes AS descuento, p.pordes,
 	p.poruti, p.iddato, g.descripda AS grupo, m.descripda AS marca, 
 	u.descripda as unidad, i.artsgrupo, s.descripda as subgrupo 
   FROM preciosv p
...
CREATE VIEW vsaldosb AS
    SELECT s.pdocode, s.idcuenta, s.saldoi, c.nucuban, c.numaux, b.descripda AS banco 
...
CREATE VIEW vsaldosban AS
    ...
    FROM saldosb s LEFT JOIN ctasban c ON s.idcuenta = c.idcuenta 
    			   LEFT JOIN detagru b ON b.iddato = c.idbanco;
...
CREATE VIEW vsaldosi AS
    SELECT i.icode, i.iname, g.descripda AS grupo, m.descripda AS marca, s.inicial, s.precio, s.iditem, s.pdocode, 
    s.idsaldosi, i.isiva, s.cospro, s.fecsaldo
    FROM (((saldosi s LEFT JOIN items i ON ((s.iditem = i.iditem))) LEFT JOIN detagru m ON ((i.artmarca = m.iddato))) LEFT JOIN detagru g ON ((i.artgrupo = g.iddato))) ORDER BY g.descripda, m.descripda, i.iname, i.icode;
...
CREATE VIEW vsuj AS 
 SELECT s.idsujeto, s.scode, s.sname, s.ssri, s.sruc, s.scedula, s.spasaporte, s.isempleado, s.isproductor, s.isproveedor, 
 		s.iscliente, s.issocio, c.descripda AS ciudad, p.descripda AS provincia, s.diasplz, s.scredito, 
...
CREATE VIEW vsujetos AS
    ...
    s.idsujeto, s.diasplz, s.scredito, s.codprin, s.saddr2, x.descripda as ntipdocextraj
...
CREATE OR REPLACE VIEW vsecuagen AS 
 SELECT s.codigo, s.idagencia, t.idtablasis, s.secini, s.secfin, s.fecini, s.ismaster, t.tabla, t.tienerep, 
 		t.nivel, t.ckey, t.secsis, a.descripda AS agencia
   FROM secuagen s
   LEFT JOIN tablasis t ON s.idtablasis = t.idtablasis
   LEFT JOIN detagru a ON s.idagencia = a.iddato;

ALTER TABLE vsecuagen OWNER TO postgres;

CREATE OR REPLACE VIEW vcheques AS 
 SELECT c.numcta, c.numche, c.fecheque, c.observa, c.valor, c.fecha, b.descripda AS banco, c.ctaban, c.idcheque, c.isefectivo, c.fecdepo
   FROM cheques c
